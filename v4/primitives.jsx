...
// ─── User mode context (demo selector) ────────────────────────────
// Three states of relationship with Márcio. Internal naming only —
// the app never labels the user. The chip is for the pitch demo.
const MODES = ['curioso', 'praticante', 'discipulo'];
const MODE_LABEL = { curioso: 'CURIOSO', praticante: 'PRATICANTE', discipulo: 'DISCÍPULO' };
...
const useMode = () => React.useContext(UserModeContext);

// ─── Recorte (pull-quote from a past Márcio consultation) ─────────
// The product's editorial moat: phrases marked by user/Márcio/topic,
// surfaced inside Hoje and Mapa to thread Márcio's voice across the app.
function Recorte({ from = 'MAR/26', children, compact = false }) {
  return (
    <div style={{
      ...
      <div style={{
        ...
        color: V2.terracotta, textTransform: 'uppercase',
        marginBottom: 5,
      }}>MÁRCIO TE DISSE · {from}</div>
      <div style={{
        fontFamily: TYPE.italic, fontStyle: 'italic',
        fontSize: compact ? 13 : 14, color: V2.walnut, lineHeight: 1.4,
      }}>“{children}”</div>
    </div>
  );
}

// ─── Lock glyph for paywalled content ─────────────────────────────
function LockGlyph({ size = 14, color = V2.walnut }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" stroke={color} strokeWidth="1.4" strokeLinejoin="round" strokeLinecap="round">
      <rect x="3" y="7" width="10" height="7" rx="1"></rect>
      <path d="M 5 7 L 5 5 Q 5 2.5, 8 2.5 Q 11 2.5, 11 5 L 11 7"></path>
    </svg>
  );
}

// ─── WhatsApp glyph (simple, hand-drawn feel) ─────────────────────
function WhatsappGlyph({ size = 18, color = V2.walnut }) {
  return (
    <svg width={size} height={size} viewBox="0 0 22 22" fill="none" stroke={color} strokeWidth="1.4" strokeLinejoin="round" strokeLinecap="round">
      <path d="M 3 19 L 5 14 Q 2.5 9, 6 5 Q 11 1.5, 16 5 Q 20 9, 17 14 Q 13 19, 8 17 Z"></path>
    </svg>
  );
}

// ─── Calendar glyph ───────────────────────────────────────────────
function CalendarGlyph({ size = 18, color = V2.walnut }) {
  return (
    <svg width={size} height={size} viewBox="0 0 22 22" fill="none" stroke={color} strokeWidth="1.3" strokeLinejoin="round" strokeLinecap="round">
      <rect x="3" y="5" width="16" height="14" rx="1"></rect>
      <path d="M 7 3 L 7 7"></path><path d="M 15 3 L 15 7"></path>
      <path d="M 3 9 L 19 9"></path>
    </svg>
  );
}
...
Object.assign(window, {
  ...
  MODES, MODE_LABEL, UserModeContext, useMode, Recorte, LockGlyph, WhatsappGlyph, CalendarGlyph,
});
